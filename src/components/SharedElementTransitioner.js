...
const SATUSBAR_HEIGHT = (Platform.OS === 'android' && Platform.Version >= 21) ? 24 : 0;
...
class SharedElementTransitioner extends Component {
    ...
    shouldComponentUpdate(nextProps, nextState: State) {
        ...
        */
        return this.maunted && (this.props !== nextProps || nextState.itemsToMeasure.length === 0);
    }
    ...
    _getSharedElementStyle(props, prevProps, itemFrom, itemTo) {
        ...
        const animateScale = (itemFrom, itemTo) => {
            ...
            const translateX = progress.interpolate({
                inputRange: [0, 1],
                outputRange: [0, (itemTo.metrics.x + itemTo.metrics.width/2) - (itemFrom.metrics.x + itemFrom.metrics.width/2)],
            });
            const translateY = progress.interpolate({
                inputRange: [0, 1],
                outputRange: [0, (itemTo.metrics.y + itemTo.metrics.height/2) - (itemFrom.metrics.y + itemFrom.metrics.height/2)],
            });
            return {
                transform: [
        ...
        const left = itemFrom.metrics.x;
        const top = itemFrom.metrics.y;
    ...
    _renderFakedSEContainer(pairs, props, prevProps) {
        ...
        const left = index > prevIndex ? fromItemBBox.left : toItemBBox.left;
        const top = index > prevIndex ? fromItemBBox.top : toItemBBox.top;
        const width = index > prevIndex ? fromItemBBox.width : toItemBBox.width;
        const height = index > prevIndex ? fromItemBBox.height : toItemBBox.height;

        const translateX = position.interpolate({
            inputRange,
            outputRange: [0, (windowWidth/2 - (left + width/2))],
        });

        const translateY = position.interpolate({
            inputRange,
            outputRange: [0, (windowHeight/2 - (top + height/2))],
        });

        const scaleX =  position.interpolate({
            inputRange,
            outputRange: [1, windowWidth/width],
        });

        const scaleY =  position.interpolate({
            inputRange,
            outputRange: [1, windowHeight/height],
        });
    ...
    _renderDarkeningOverlay(progress, position, sceneIndex: number) {
        ...
        const animatedStyle = {
            zIndex: 5, // to ensure the overlay covers toolbar
